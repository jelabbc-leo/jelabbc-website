...
function cargarDatosComunicado(d) {
    // NOTA: cboEntidad eliminado - La entidad se maneja automáticamente desde la sesión
    if (cboSubEntidad) cboSubEntidad.SetValue(d.SubEntidadId);
...
function guardarComunicado() {
    ...
    if (!datos.titulo || !datos.contenido || !datos.fechaPublicacion) {
...
function onEntidadChanged(s, e) {
    console.log('onEntidadChanged: Función obsoleta - el sistema multi-entidad maneja esto automáticamente');
}
